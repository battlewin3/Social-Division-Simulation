...
# Note: plyr has been retired. All bind_rows() calls replaced with dplyr::bind_rows().
...
fulloutput<-lapply(loopdf$i,function(i) {
  ...
  #params
  set.seed(thisrow$seed * 10000 + i) #unique seed per (seed, i) combination
...
#record runtime and session info
runtime_mins <- difftime(Sys.time(), start_time, units="mins")
cat("Simulation completed in", round(runtime_mins, 1), "minutes.\n")
sink(file.path(filesdir, "session_info.txt"))
cat("Simulation date:", as.character(Sys.time()), "\n")
cat("Runtime (minutes):", round(runtime_mins, 1), "\n\n")
sessionInfo()
sink()
cat("Session info saved to files/session_info.txt\n")
